...
int main(){
    std::thread t1(addMoney); //either pf the thread will reach cpu first and lock first 
    std::thread t2(addMoney); //so other thread will be blocked unlick the first thread unlocks
